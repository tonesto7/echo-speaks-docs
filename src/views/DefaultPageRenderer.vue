<template>
  <div class="container" @scroll="handleScroll">
    ...
    <section class="banner" :class="{ active: isActive }" v-if="pageConfig && !pageConfig.tiles" :data-name="pageConfig.name">
      <h2 class="banner__title"><icon :name="pageConfig.icon" v-if="pageConfig.icon"></icon> {{ pageConfig.name }}</h2>
      <h4 class="banner__text">{{ pageConfig.description }}</h4>
      <p class="banner__timestamp" style="margin-bottom: 0;">{{ lastModified }}</p>
    ...
    <ul class="cards" v-if="pageConfig.tiles">
      ...
      >
        <router-link :to="tile.path">
          <icon class="card__icon" scale="2" :name="tile.icon" v-if="tile.icon && tile.icon !== 'watson'"></icon>
    ...
    initialize (path) {
      ...
      if (this.pageConfig.markdown !== undefined) {
        ...
        axios.get(path, config).then(response => {
          this.lastModifiedDt = response.headers['last-modified']
          if (this.lastModifiedDt) {
            let dt = Date.parse(this.lastModifiedDt.toString())
            let newDt = new Date(dt)
            this.lastModified = `Modified: ${newDt.toLocaleString()}`
            // console.log(this.lastModified)
          } else {
            this.lastModified = 'Document Last Modified: Unknown'
          }
